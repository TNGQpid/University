...
# Model: Chat-GPT 4-o, Prompt: Add on to my current heuristic to scan for 3-in-a-row sequences
# Model: Chat-GPT 4-o, Prompt: My check for diagonol wins is incorrect, fix it
...
class TeekoPlayer:
    ...
    def make_move(self, state):
        ...
        def minimax(statee, max_depth=3):
            ...
            def heuristic(state):
                """
                Advanced heuristic for Teeko:
                - Favor creating 3-in-a-row sequences.
                - Penalize leaving opponent close to winning.
                - Encourage central control and clustering.
                - Evaluate based on game state dynamics.
                """
                score = 0
                
                # Check for 2-in-a-row and 3-in-a-row horizontally
                for row in state:
                    for i in range(3):
                        segment = row[i:i+3]
                        if segment.count(self.my_piece) == 2 and segment.count(' ') == 1:
                            score += 20  # More weight for advancing our position
                        if segment.count(self.opp) == 2 and segment.count(' ') == 1:
                            score -= 20
                        if segment.count(self.my_piece) == 3:
                            score += 50  # Close to winning is highly favorable
                        if segment.count(self.opp) == 3:
                            score -= 50  # Opponent close to winning is highly unfavorable

                # Vertical evaluation
                for col in range(5):
                    for i in range(3):
                        column_segment = [state[row][col] for row in range(i, i+3)]
                        ...
                        if column_segment.count(self.opp) == 2 and column_segment.count(' ') == 1:
                            score -= 20
                        if column_segment.count(self.my_piece) == 3:
                            score += 50
                        if column_segment.count(self.opp) == 3:
                            score -= 50

                # Diagonal evaluation
                for i in range(3):
                    for j in range(3):
                        diagonal_segment1 = [state[i+k][j+k] for k in range(3)]
                        diagonal_segment2 = [state[i+k][j+2-k] for k in range(3)]
                        for segment in [diagonal_segment1, diagonal_segment2]:
                            if segment.count(self.my_piece) == 2 and segment.count(' ') == 1:
                                score += 20
                            if segment.count(self.opp) == 2 and segment.count(' ') == 1:
                                score -= 20
                ...
                # Apply dynamic scaling for stability
                score = math.tanh(score / 150)  # Normalize within (-1, 1)
            ...
            for s in get_succ(statee):
                if time.time() - start_time > 4.5:  # Check time limit
                    break
